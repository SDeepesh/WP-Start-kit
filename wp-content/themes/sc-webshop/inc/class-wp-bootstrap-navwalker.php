...
if ( ! class_exists( 'WP_Bootstrap_Navwalker' ) ) {
	 ...
	 */
	class WP_Bootstrap_Navwalker extends Walker_Nav_Menu { // phpcs:ignore
		...
		public function start_lvl( &$output, $depth = 0, $args = array() ) {
			 ...
			 * @param int      $depth   Depth of menu item. Used for padding.
			 */
			$class_names = join( ' ', apply_filters( 'nav_menu_submenu_css_class', $classes, $args, $depth ) ); // phpcs:ignore WordPress.NamingConventions.PrefixAllGlobals.NonPrefixedHooknameFound
		...
		public function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
			...
			if ( isset( $args->has_children ) && $args->has_children && 0 === $depth && $args->depth > 1 ) {
				...
				$atts['id']            = 'menu-item-dropdown-' . $item->ID; // phpcs:ignore Generic.Formatting.MultipleStatementAlignment.IncorrectWarning
			} else {
			...
			$title = apply_filters( 'the_title', $item->title, $item->ID ); // phpcs:ignore WordPress.NamingConventions.PrefixAllGlobals.NonPrefixedHooknameFound
		...
		public static function fallback( $args ) {
			if ( current_user_can( 'edit_theme_options' ) ) {
				...
				$fallback_output .= '>';
				$fallback_output .= '<li class="nav-item"><a href="' . esc_url( admin_url( 'nav-menus.php' ) ) . '" class="nav-link" title="' . esc_attr__( 'Add a menu', 'sc-webshop' ) . '">' . esc_html__( 'Add a menu', 'sc-webshop' ) . '</a></li>';
				$fallback_output .= '</ul>';
				if ( $container ) {
					$fallback_output .= '</' . esc_attr( $container ) . '>';
				}
